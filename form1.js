...
const simpleSelect = (self, label, values, multiple) => {
  // if (!multiple && localStorage.getItem(label) && !self.state[label])
  //   self.setState({ [label]: localStorage.getItem(label) })
  return <FormControl className="textview">
    <InputLabel className="inputlabel">{`${label}${!mandatory.includes(label) ? '' : ' *'}`}</InputLabel>
    <Select
      value={self.state[label] || (multiple ? [] : /*localStorage.getItem(label) ||*/ '')}
      onChange={e => {
        self.setState({ [label]: e.target.value })
        // if (!multiple)
        //   localStorage.setItem(label, e.target.value)
    ...
    return (
      <div className="Home">
        <div className="header">
          ...
          <div className="helem"><h3>Feuille de prod: Monoligne</h3></div>
        ...
        <div className="body">
          ...
          <div className="container" >
            {/* {simpleTextView(this, "Offre de départ")} */}
            {/* <TextField className="textview"
              ...
              rowsMax={6}
            /> */}
            {simpleSelect(this, "OFFRE DE DEPART PARTIE FIXE", [
              "CONCURRENCE",
              "ORANGE PARTICULIER",
              "OPTIMAL PRO",
              "LIVEBOX PRO ADSL",
            ...
            {simpleSelect(this, "Offre d'arrivée fixe", [
              ...
              "LIVEBOX PRO FIBRE 2 LIGNES",
              "OPEN PRO FIBRE",
              "OPEN PRO FIBRE 2 LIGNE",
              "OPEN PRO ALL IP",
              "OPEN PRO ALL 2 LIGNES"
            ])}
          </div>
          <div className="container">
                ...
                this.setState({ submit: true }, () => {
                  ...
                  this.setState({ missing: { ...missing }, error: false }, () => {
                    if (Object.keys(missing).length === 0) {
                      Api.postForm1(this.state).then((success) => {
